      ...
      case TYPE_ARRAY:
        ...
        else
        {
          ...
          if (type_array_is_decl_as_ptr(t))
            write_pointer_left(h, type_array_get_element(t)->type);
        }
        break;
      case TYPE_BASIC:
...
void write_type_right(FILE *h, type_t *t, int is_field)
{
  ...
  case TYPE_ARRAY:
  {
    type_t *elem = type_array_get_element(t)->type;
    if (type_array_is_decl_as_ptr(t))
    {
...
unsigned int get_context_handle_offset( const type_t *type )
{
    ...
    {
        if (type_is_alias( type )) type = type_alias_get_aliasee( type )->type;
        ...
        if (iface_stmts) LIST_FOR_EACH_ENTRY( iface_stmt, iface_stmts, statement_t, entry )
        {
            ...
            for (typedef_entry = iface_stmt->u.typedef_list; typedef_entry; typedef_entry = typedef_entry->next)
            {
                if (!is_attr(typedef_entry->var->declspec.type->attrs, ATTR_ENCODE)
                    ...
                    continue;
                if (!proc(header, typedef_entry->var->declspec.type))
                    return 0;
            }
        }
    }
...
void write_args(FILE *h, const var_list_t *args, const char *name, int method, int do_indent)
{
  ...
  if (args) LIST_FOR_EACH_ENTRY( arg, args, const var_t, entry ) {
    ...
    write_decltype_decl(h, (decl_type_t*)&arg->declspec, arg->name);
    if (method == 2) {
  ...
  STATEMENTS_FOR_EACH_FUNC(stmt, type_iface_get_stmts(iface))
  {
    ...
    if (!is_callas(func->attrs)) {
      ...
      if (is_aggregate_return(func)) {
        ...
        write_decltype_decl_left(header, type_function_get_retdeclspec(func->declspec.type));
  ...
  STATEMENTS_FOR_EACH_FUNC(stmt, type_iface_get_stmts(iface)) {
    ...
    if (cas) {
      ...
      if (&stmt2->entry != type_iface_get_stmts(iface)) {
        ...
        write_decltype_decl_left(fp, type_function_get_retdeclspec(m->declspec.type));
        ...
        write_decltype_decl_left(fp, type_function_get_retdeclspec(func->declspec.type));
...
static void write_rpc_interface_start(FILE *header, const type_t *iface)
{
  ...
  {
      fprintf(header, "extern ");
      write_decltype_decl( header, (decl_type_t*)&var->declspec, var->name );
      fprintf(header, ";\n");
  }
  if (old_names)
  {
    ...
    switch (stmt->type)
    {
      ...
      case STMT_TYPEDEF:
      {
        ...
        for (; typedef_entry; typedef_entry = typedef_entry->next)
	        write_typedef(header, typedef_entry->var->declspec.type);
        break;
      }
      case STMT_LIBRARY:
