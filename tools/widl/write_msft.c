	...
	int *decoded_size)         /* [O] The total size of the unencoded TYPEDESCs, including nested descs. */
{
	...
	*decoded_size = 8 /*sizeof(TYPEDESC)*/ + child_size;
    ...
    case VT_SAFEARRAY:
	{
	type_t *element_type = type_alias_get_aliasee(type_array_get_element(type))->type;
	int next_vt = get_type_vt(element_type);

	encode_type(typelib, next_vt, type_alias_get_aliasee(type_array_get_element(type))->type,
        &target_type, &child_size);
    ...
    *decoded_size = 0;
    ...
    if (vt == VT_PTR) {
        type_t *ref = is_ptr(type) ?
            type_pointer_get_ref(type)->type : type_array_get_element(type);
	...
	if (typeoffset == typelib->typelib_segdir[MSFT_SEG_TYPEDESC].length) {
		...
		type_t *element_type = type_alias_get_aliasee(type_array_get_element(ref))->type;
