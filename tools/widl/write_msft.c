	...
	int *decoded_size)         /* [O] The total size of the unencoded TYPEDESCs, including nested descs. */
{
	...
	*encoded_type = 0x80000000 | (VT_EMPTY << 16) | vt;
    ...
    case VT_PTR:
      {
        int next_vt;
        for(next_vt = 0; is_ptr(type); type = type_pointer_get_ref(type)->type) {
            next_vt = get_type_vt(type_pointer_get_ref(type)->type);
            if (next_vt != 0)
                break;
        }
        ...
        encode_type(typelib, next_vt, type_pointer_get_ref(type)->type,
	...
	*decoded_size = 8 /*sizeof(TYPEDESC)*/ + child_size;
    ...
    case VT_SAFEARRAY:
	{
	type_t *element_type = type_alias_get_aliasee(type_array_get_element(type)->type)->type;
	int next_vt = get_type_vt(element_type);

	encode_type(typelib, next_vt, type_alias_get_aliasee(type_array_get_element(type)->type)->type,
        &target_type, &child_size);
    ...
    case VT_USERDEFINED:
      {
        ...
        else
        {
            ...
            while (type_is_alias(type) && !is_attr(type->attrs, ATTR_PUBLIC))
                type = type_alias_get_aliasee(type)->type;
    ...
    *decoded_size = 0;
    ...
    if (is_array(type) && !type_array_is_decl_as_ptr(type)) {
        ...
        for (atype = type;
             ...
             atype = type_array_get_element(atype)->type)
            ++num_dims;
    ...
    if (vt == VT_PTR) {
        type_t *ref = is_ptr(type) ?
            type_pointer_get_ref(type)->type : type_array_get_element(type)->type;
	...
	if (typeoffset == typelib->typelib_segdir[MSFT_SEG_TYPEDESC].length) {
		...
		type_t *element_type = type_alias_get_aliasee(type_array_get_element(ref)->type)->type;
...
static void write_default_value(msft_typelib_t *typelib, type_t *type, expr_t *expr, int *out)
{
    ...
    if (type_get_type(type) == TYPE_ENUM) {
        ...
        vt = get_type_vt(type_pointer_get_ref(type)->type);
...
static void add_typedef_typeinfo(msft_typelib_t *typelib, type_t *tdef)
{
    ...
    type = type_alias_get_aliasee(tdef)->type;
...
static void add_entry(msft_typelib_t *typelib, const statement_t *stmt)
{
    ...
    case STMT_TYPEDEF:
    {
        ...
        for (; typedef_entry; typedef_entry = typedef_entry->next) {
            ...
            if (is_attr(typedef_entry->var->declspec.type->attrs, ATTR_PUBLIC))
                add_typedef_typeinfo(typelib, typedef_entry->var->declspec.type);
            else
                add_type_typeinfo(typelib, type_alias_get_aliasee(typedef_entry->var->declspec.type)->type);
        }
        break;
    }
    case STMT_MODULE:
