...
static void clear_output_vars( const var_list_t *args )
{
  ...
  LIST_FOR_EACH_ENTRY( arg, args, const var_t, entry )
  {
      ...
      {
          if (type_get_type(type_pointer_get_ref(arg->declspec.type)->type) == TYPE_BASIC) continue;
          if (type_get_type(type_pointer_get_ref(arg->declspec.type)->type) == TYPE_ENUM) continue;
      }
                      ...
                      unsigned int proc_offset)
{
  ...
  if (is_interpreted_func( iface, func ))
  {
      ...
      write_decltype_decl_left(proxy, &retval->declspec);
  ...
  write_decltype_decl_left(proxy, (decl_type_t*)&retval->declspec);
  print_proxy( " %s %s_%s_Proxy(\n", callconv, iface->name, get_name(func));
  write_args(proxy, args, iface->name, 1, TRUE);
  print_proxy( ")\n");
  print_proxy( "{\n");
  ...
  if (has_ret) {
    print_proxy( "%s", "" );
    write_decltype_decl(proxy, &retval->declspec, retval->name);
    fprintf( proxy, ";\n" );
  }
